...
For repository operations, current Git and GitHub state is authoritative.
Canonical `git` and `gh` observations are used when installed, authorized, and
sufficient. Optional helpers, connectors, cached descriptions, and conversation
memory neither replace that state nor create a blocker when the canonical path
succeeds. Genuine failures use the installed protocol's blocker categories.
...
Governance version identity includes the explicitly supported source layouts.
Where a released version was genuinely generated in more than one layout, a
migration declares provenance and file-level evidence for each variant and must
select exactly one. Zero or ambiguous matches are authoritative-state conflicts;
same-version layout differences are never accepted implicitly.

The installed Draft 2020-12 schema validates machine-object structure. Separate
deterministic checks validate protocol vocabulary, cross-file consistency,
migration integrity, and handoff digests. Both layers must pass before a
governance or governed-handoff ready tag is created.
